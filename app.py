...
async def process_prompt(prompt):
    image_keywords = ["generate", "image", "picture", "photo", "visualize", "draw"]
    return any(keyword in prompt.lower() for keyword in image_keywords)

async def generate_image(prompt):
    try:
        azure_openai_client = await init_openai_client()
        response = await azure_openai_client.images.generate(
            model="dall-e-3", 
            prompt=prompt,
            n=1
        )
        ...
        logging.exception("Exception while generating image", e)
        return None
...
async def send_chat_request(request_body, request_headers):
    ...
    try:
        ...
        if await process_prompt(prompt):
            image_url = await generate_image(prompt)
            if image_url:
                response = {"text": "Here is the generated image:", "image_url": image_url}
                apim_request_id = None
            else:
                response = {"text": "Failed to generate image."}
                apim_request_id = None
        else:
            azure_openai_client = await init_openai_client()
